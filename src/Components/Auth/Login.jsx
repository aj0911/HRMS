...
import { Constants, compareEncryptedData, validateEmail } from '../../Helper/Helper';
            ...
            try{
                onValue(ref(db,`users/`), (snapshot) => {
                    ...
                    Object.values(data).forEach(user=>{
                        if(user.email.toLocaleLowerCase()===userData.email.toLocaleLowerCase()){
                            flag = true;
                            if(compareEncryptedData(userData.pass,user.pass)){
                                dispatch(login({user,remember:checkbox.current.checked}))
                                setAlertData({show:true,type:0,heading:'Login Successfully',subHeading:'You have logged in to your account successfully',onSubmit:()=>{
                                    setAlertData({show:false})
                                    navigate('/')
                                },btnText:'Go to Dashboard'});
                            }
                            else{
                                setAlertData({show:true,type:1,heading:'Login Failed',subHeading:'Incorrect Password',onSubmit:()=>{
                                    setAlertData({show:false})
                                },btnText:'Try Again'});
                            }
                        }
                    ...
                    if(!flag){
                        setAlertData({show:true,type:1,heading:'Login Failed',subHeading:'Email is not Registerd yet!',onSubmit:()=>{
                            setAlertData({show:false})
                        },btnText:'Try Again'});
                    }
                    setLoader(prev => !prev)
                ...
                setAlertData({show:true,type:1,heading:'Login Failed',subHeading:error.message,onSubmit:()=>{
                    setAlertData({show:false})
                },btnText:'Try Again'});
                setLoader(prev=>!prev)
            }
        ...
        <div className="login">
            ...
            <form onSubmit={(e)=>handleLogin(e)}>
                <div className="inputControl">
                    <label onClick={e=>handleFocus(e)} htmlFor="email">Email Address</label>
                    <input onChange={e=>setUserData({...userData,email:e.target.value})} type="email" name="email"/>
                </div>
                ...
                <div className="control">
                    <div className="remember">
                        ...
                        <label onClick={()=>{checkbox.current.checked = !checkbox.current.checked}} htmlFor="remember">Remember Me</label>
                    </div>
